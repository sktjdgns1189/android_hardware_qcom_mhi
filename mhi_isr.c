...
irqreturn_t irq_cb(int irq_number, void *dev_id)
{
	...
	if (NULL == mhi_dev_ctxt) {
		mhi_log(MHI_MSG_ERROR, "%s",
			"Failed to get a proper context\n");
		return IRQ_HANDLED;
	}
		...
		u32 event_quota)
{
	...
	while ((local_rp != device_rp) && (event_quota > 0) &&
		...
		for (i = 0;
		     i < 1000 && ((mhi_xfer_event_pkt *)local_rp)->xfer_ptr == 0xDEADBEEF;
		     ++i) {};
		if (i != 0)
			mhi_log(MHI_MSG_ERROR,
				"Event was not valid, retried %d times\n", i);
		event_to_process = *local_rp;
		((mhi_xfer_event_pkt *)local_rp)->xfer_ptr = 0xDEADBEEF;
		if (unlikely(MHI_STATUS_SUCCESS != recycle_trb_and_ring(mhi_dev_ctxt,
						local_ev_ctxt,
						MHI_RING_TYPE_EVENT_RING,
						ev_index)))
			mhi_log(MHI_MSG_ERROR, "%s", "Failed to recycle ev pkt\n");
...
mhi_result *mhi_poll(mhi_client_handle *client_handle)
{
	...
	if (MHI_STATUS_SUCCESS != ret_val)
		mhi_log(MHI_MSG_INFO, "%s", "NAPI failed to process event ring\n");
...
void mhi_mask_irq(mhi_client_handle *client_handle)
{
	...
	client_handle->mhi_dev_ctxt->msi_disable_cntr++;
	if (client_handle->mhi_dev_ctxt->msi_disable_cntr >
		   (client_handle->mhi_dev_ctxt->msi_enable_cntr + 1))
		mhi_log(MHI_MSG_INFO, "%s", "No nested IRQ disable Allowed\n");
}
void mhi_unmask_irq(mhi_client_handle *client_handle)
{
	client_handle->mhi_dev_ctxt->msi_enable_cntr++;
	enable_irq(MSI_TO_IRQ(client_handle->mhi_dev_ctxt,
			client_handle->msi_vec));
	if (client_handle->mhi_dev_ctxt->msi_enable_cntr >
		   client_handle->mhi_dev_ctxt->msi_disable_cntr)
		mhi_log(MHI_MSG_INFO, "%s", "No nested IRQ enable Allowed\n");
}
