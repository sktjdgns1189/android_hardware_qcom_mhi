...
MHI_STATUS mhi_test_for_device_ready(mhi_device_ctxt *mhi_dev_ctxt)
{
	u32 pcie_word_val = 0;
	u32 expiry_counter;
	mhi_log(MHI_MSG_INFO, "%s", "Waiting for MMIO Ready bit to be set\n");
...
MHI_STATUS mhi_init_mmio(mhi_device_ctxt *mhi_dev_ctxt)
{
	...
	mhi_log(MHI_MSG_INFO, "%s", "~~~ Initializing MMIO ~~~\n");
	...
	if (0 == mhi_dev_ctxt->mmio_len) {
		mhi_log(MHI_MSG_ERROR, "%s", "Received mmio length as zero\n");
		return MHI_STATUS_ERROR;
	}

	mhi_log(MHI_MSG_INFO, "%s", "Testing MHI Ver\n");
	...
	mhi_log(MHI_MSG_INFO, "%s", "Setting all MMIO values.\n");
