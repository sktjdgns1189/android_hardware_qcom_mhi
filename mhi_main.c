...
int mhi_init_pcie_device(mhi_pcie_dev_info *mhi_pcie_dev)
{
	...
	mhi_log(MHI_MSG_INFO, "%s", "Successfully enabled pcie device.\n");
	...
	if (0 == mhi_pcie_dev->core.bar0_base) {
		mhi_log(MHI_MSG_ERROR,
			"%s", "Failed to register for pcie resources\n");
	...
	if (ret_val)
		mhi_log(MHI_MSG_ERROR, "%s", "Could not request BAR0 region\n");
	...
	if (mhi_pcie_dev->core.manufact_id != MHI_PCIE_VENDOR_ID ||
		...
		mhi_log(MHI_MSG_ERROR, "%s",
			"Incorrect device/manufacturer ID\n");
...
int mhi_init_gpios(mhi_pcie_dev_info *mhi_pcie_dev)
{
	...
	if (!of_find_property(np, "mhi-device-wake-gpio", NULL))
		...
		mhi_log(MHI_MSG_VERBOSE, "%s", "Found gpio as a property in DT\n");
	ret_val = of_get_named_gpio(np, "mhi-device-wake-gpio", 0);
	switch (ret_val) {
	case -EPROBE_DEFER:
		mhi_log(MHI_MSG_VERBOSE, "%s", "DT is not ready\n");
		return ret_val;
		break;
	case 0:
		mhi_log(MHI_MSG_CRITICAL, "%s",
	...
	if (ret_val) {
		mhi_log(MHI_MSG_VERBOSE,
			"%s", "Could not obtain device WAKE gpio\n");
	}
	mhi_log(MHI_MSG_VERBOSE,
		"%s",
		...
		uintptr_t buf, size_t buf_len, u32 chain, u32 eob)
{
	...
	if (NULL == client_handle || !VALID_CHAN_NR(client_handle->chan) ||
		...
		mhi_log(MHI_MSG_CRITICAL, "%s", "Bad input args\n");
		return MHI_STATUS_ERROR;
	}
	...
	if (unlikely(MHI_STATUS_SUCCESS != ret_val)) {
		mhi_log(MHI_MSG_INFO, "%s", "Failed to insert trb in xfer ring\n");
			...
			MHI_COMMAND cmd, u32 chan)
{
	...
	default:
		mhi_log(MHI_MSG_ERROR, "%s", "Bad command received\n");
	}
	...
	if (MHI_STATUS_SUCCESS !=
			...
			(void *)&cmd_pkt)) {
		mhi_log(MHI_MSG_ERROR, "%s", "Failed to insert element\n");
		goto error_general;
	}
...
MHI_STATUS parse_xfer_event(mhi_device_ctxt *ctxt, mhi_event_pkt *event)
{
	...
	default:
		{
			mhi_log(MHI_MSG_ERROR, "Unknown TX completion.\n");
			break;
		}
	} /*switch(MHI_EV_READ_CODE(EV_TRB_CODE,event)) */
	return 0;
		...
		u32 ring_index)
{
	...
	if (NULL == mhi_dev_ctxt || NULL == ring ||
		...
		mhi_log(MHI_MSG_ERROR, "%s", "Bad input params\n");
		return ret_val;
	}
	ret_val = ctxt_del_element(ring, &removed_element);
	if (MHI_STATUS_SUCCESS != ret_val) {
		mhi_log(MHI_MSG_ERROR, "%s", "Could not remove element from ring\n");
		return MHI_STATUS_ERROR;
	}
	ret_val = ctxt_add_element(ring, &added_element);
	if (MHI_STATUS_SUCCESS != ret_val)
		mhi_log(MHI_MSG_ERROR, "%s", "Could not add element to ring\n");
	db_value = mhi_v2p_addr(mhi_dev_ctxt->mhi_ctrl_seg_info,
				(uintptr_t)ring->wp);
	if (MHI_STATUS_SUCCESS != ret_val)
		return ret_val;
	...
	if ((MHI_STATE_M0 == mhi_dev_ctxt->mhi_state ||
		...
		default:
			mhi_log(MHI_MSG_ERROR, "%s", "Bad ring type\n");
		}
	}
	atomic_dec(&mhi_dev_ctxt->data_pending);
	return ret_val;
...
MHI_STATUS parse_cmd_event(mhi_device_ctxt *mhi_dev_ctxt, mhi_event_pkt *ev_pkt)
{
	...
	cmd_pkt = (mhi_cmd_pkt *)mhi_p2v_addr(mhi_dev_ctxt->mhi_ctrl_seg_info,
							phy_trb_loc);
	mhi_log(MHI_MSG_INFO, "%s", "Received CMD completion event\n");
	...
	case MHI_EVENT_CC_SUCCESS:
	{
		...
		{
			mhi_log(MHI_MSG_INFO, "%s", "Processed cmd stop event\n");
			if (MHI_STATUS_SUCCESS != ret_val) {
				mhi_log(MHI_MSG_INFO, "%s",
		...
		mhi_log(MHI_MSG_INFO, "%s", "CMD completion indicated successful\n");
		mhi_dev_ctxt->mhi_chan_pend_cmd_ack[chan] = MHI_CMD_NOT_PENDING;

		break;
	}
	default:
		mhi_log(MHI_MSG_INFO, "%s", "Unhandled mhi completion code\n");
		break;
	}
	mhi_log(MHI_MSG_INFO, "%s", "Recycling cmd event\n");
...
MHI_STATUS reset_chan_cmd(mhi_device_ctxt *mhi_dev_ctxt, mhi_cmd_pkt *cmd_pkt)
{
	...
	chan_ctxt = &mhi_dev_ctxt->mhi_ctrl_seg->mhi_cc_list[chan];
	mhi_log(MHI_MSG_INFO, "%s", "Processed cmd reset event\n");
...
MHI_STATUS start_chan_cmd(mhi_device_ctxt *mhi_dev_ctxt, mhi_cmd_pkt *cmd_pkt)
{
	...
	mhi_log(MHI_MSG_INFO, "%s", "Processed cmd channel start\n");
	return MHI_STATUS_SUCCESS;
}
...
MHI_STATUS probe_clients(mhi_device_ctxt *mhi_dev_ctxt)
{
	...
	if (ret_val)
		mhi_log(MHI_MSG_CRITICAL, "%s",
			"Failed to add MHI_RmNET device.\n");
